...
extern char lcd_msg_var[16];
extern uint8_t test;
// Flag for receive reply interrupt
extern uint8_t flag_var;
...
/*    
    char lcd_msg_var[16];
    sprintf(lcd_msg_var, "recv data: %d", data);
    lcd_msg_delayed(lcd_msg_var);
*/
...
ISR(W_INT_VECT)
{
   ...
   /*---*/
   	...
   	if (int_val & IR_UNREACH)
   	{
        ...
        lcd_msg_delayed("IP Unreachable");
   	}
      ...
      /*---*/
      /*
      sprintf(lcd_msg_var, "I_STATUS[0]: %02X", I_STATUS[0]);
      lcd_msg_delayed(lcd_msg_var);*/
      
      // Code for replying to a received message
      if (I_STATUS[0] & Sn_IR_RECV)
      {
          flag_var = 1;
          /*
          lcd_msg_delayed("recv int home");         // Debug
          sprintf(lcd_msg_var, "S0_IR: %02X", IINCHIP_READ(Sn_IR(0)));
          lcd_msg_delayed(lcd_msg_var);    */
      }
