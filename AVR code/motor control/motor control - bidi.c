...
int main(void)
{
    ...
    ip_var[3] = 0;
    setSUBR(ip_var);
    uint8_t mac[] = {0x8A, 0x26, 0xE0, 0x1A, 0xDD, 0x83};
    setSHAR(mac);
    ip_var[0] = 192;
    ip_var[1] = 168;
    ip_var[2] = 0;
    ...
    */
    ...
    //sendto(0, (uint8_t *)"hello\0", 6, peer_ip, 54321);
    ...
    while(1)
    {
        if (IINCHIP_READ(Sn_IR(0)) & Sn_IR_RECV)
        {
            ...
            if (/*(sender_ip[3]==5) &&*/ (sender_port==MATLAB_PWM_PORT))
            {
                // Debug
                //sendto(0, (uint8_t *)"here0", 5, peer_ip, 54321);
                
                if (duty_cycle!=prev_duty)
                {
                    // Debug
                    //sendto(0, (uint8_t *)"herei", 5, peer_ip, 54321);
                    
                    if (duty_cycle<0) // CW direction
                    {
                        // Debug
                        //sendto(0, (uint8_t *)"here1", 5, peer_ip, 54321);
                    ...
                    {
                        // Debug
                        //sendto(0, (uint8_t *)"here2", 5, peer_ip, 54321);
                    ...
                    {
                        // Debug
                        //sendto(0, (uint8_t *)"here3", 5, peer_ip, 54321);
                ...
                var = count_for_95ms_T0_nonstop();
                if (prev_dir==CW)
                    var = -var;
                sendto(0, (uint8_t *)&var, 2, sender_ip, MATLAB_SPEED_PORT);
            }
        }
    }
}