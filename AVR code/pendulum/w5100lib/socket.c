...
void close(SOCKET s)
{
  ...
  //lcd_msg_delayed("written");

  /* +20071122[chungs]:wait to process the command... */
  while( IINCHIP_READ(Sn_CR(s)) )
    ;
  //lcd_msg_delayed("read");
  /* ------- */
...
//  uint8 isr=0;
  ...
  printf("sendto(%s)\r\n", buf);
   ...
   //sprintf(lcd_msg_var, "ret: %02X", ret);  // Debug
  ...
  {
      lcd_msg_delayed("ip, port, failed");          // Debug
  ...
  else
  {
    ...
    /* _delay_ms(100);
    sprintf(lcd_msg_var, "Sn_DIPR1: %d", IINCHIP_READ(Sn_DIPR0(s)+1));  // Debug
    lcd_msg_delayed(lcd_msg_var);   */
...
/* +2008.01 bj */
    ...
    {
        /* Debug
        sprintf(lcd_msg_var, "getISR: %02X", getISR(s));    // getISR(x) = I_STATUS(x) = Sn_IR(x)
        lcd_msg_delayed(lcd_msg_var);   */
  ...
  if ( len > 0 )
  {
    ...
    case Sn_MR_UDP :
        ...
        *port = (*port << 8) + head[5];
        ...
        // sprintf(lcd_msg_var, "length: %d", data_len);
        // lcd_msg_delayed(lcd_msg_var);
