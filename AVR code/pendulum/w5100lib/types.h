...
#define	CLK_CPU			11059200		/**< 8Mhz(for serial) */
...
// Was giving the following error, so commented: Error	15	'EIMSK' undeclared (first use in this function)	D:\notes\FYP\Networking Chip\W5100\w5100lib\w5100lib\w5100lib.c	94	5	w5100lib
// Should be uncommented.

#if (__MCU_TYPE__ == __MCU_AVR__)
   #ifdef __DEF_IINCHIP_INT__
      // iinchip use external interrupt 1
      #define IINCHIP_ISR_DISABLE()	(GICR &= ~(1<<W_INT))
      #define IINCHIP_ISR_ENABLE()	(GICR |= (1<<W_INT))
      #define IINCHIP_ISR_GET(X)		(X = GICR)          // useless
      #define IINCHIP_ISR_SET(X)		(GICR = X)          // useless
   #else
